// src/errors.ts
var ScavioError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "ScavioError";
  }
};
var MissingAPIKeyError = class extends ScavioError {
  constructor() {
    super(
      "No API key provided. Pass apiKey or set the SCAVIO_API_KEY environment variable. Get your free key at https://dashboard.scavio.dev"
    );
...
var RateLimiter = class {
  ...
  async acquire() {
    ...
    if (this.timestamps.length >= this.maxPerSecond) {
      const sleepMs = 1e3 - (Date.now() - this.timestamps[0]);
      if (sleepMs > 0) {
  ...
  cleanup() {
    const now = Date.now();
    while (this.timestamps.length > 0 && now - this.timestamps[0] >= 1e3) {
      this.timestamps.shift();
    }
  }
};

// src/namespaces/amazon.ts
var AmazonNamespace = class {
  constructor(client) {
    this.client = client;
  }
  client;
...
// src/namespaces/walmart.ts
var WalmartNamespace = class {
  constructor(client) {
    this.client = client;
  }
  client;
